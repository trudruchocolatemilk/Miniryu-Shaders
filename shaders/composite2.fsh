...
void WaterRefraction(inout SurfaceStruct surface)
{
	if (surface.mask.water || surface.mask.ice || surface.mask.stainedGlass)
	{
		...
		*/
		...
		float visibility = 10.0f / (pow(exp(waterDepth * fogDensity), 1.0f));
...
vec3 AddCrepuscularRays(inout SurfaceStruct surface)
{
	...
	rayColor *=  colorSunlight * 0.3f + colorSkylight * 4.0f + colorSunlight * (anisoHighlight * 120.0f);
...
void main() {
	...
	surface.color.rgb += AddCrepuscularRays(surface);
	#endif
