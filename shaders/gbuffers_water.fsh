...
float GetWaves(vec3 position, in float scale) {
  ...
  float weight = WAVE_HEIGHT_MEASURE;
...
vec3 GetWaterParallaxCoord(in vec3 position, in vec3 viewVector)
{
	...
	vec3 stepSize = vec3(0.6f * WAVE_HEIGHT_MEASURE, 0.6f * WAVE_HEIGHT_MEASURE, 0.6f);
...
vec3 GetWavesNormal(vec3 position, in float scale, in mat3 tbnMatrix) {
	...
	vec3 wavesNormal;
		 ...
		 wavesNormal.r *= 20.0f * WAVE_HEIGHT_MEASURE / sampleDistance;
		 wavesNormal.g *= 20.0f * WAVE_HEIGHT_MEASURE / sampleDistance;
