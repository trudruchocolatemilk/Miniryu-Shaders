...
#define WAVING_FIRE
#define WAVING_CARROT
#define WAVING_BEETROOT_SEEDS
...
#include "include/animations.glsl"
#include "include/acid.glsl"
//#include "include/globe.glsl"
...
position.xyz += cameraPosition.xyz;
	...
	if (waveCoeff > 0.5f)
	{
		...
		float windStrength = mix(0.8f, 1.0f, rainStrength);
...
#ifdef WAVING_HAY
//haybale//
	if (mc_Entity.x == 170.0) {
		float speed = 0.1;


			  //lightWeight = max(0.0f, 1.0f - (lightWeight * 5.0f));
		
		float magnitude = (sin((position.y + position.x + tick * pi / ((4.0) * speed))) * 0.15 + 0.15) * 0.30 * lightWeight;
			  // magnitude *= grassWeight;
			  magnitude *= lightWeight;
		float d0 = sin(tick * pi / (112.0 * speed)) * 3.0 - 1.5;
		float d1 = sin(tick * pi / (142.0 * speed)) * 3.0 - 1.5;
...
//tall ass
	...
	//lower grass movement
	if (materialIDs == 175.0f) {
		float speed = 0.075;
...
#ifdef WAVING_FIRE
//these legit have no purpose why do i keep putting them here
	if (mc_Entity.x == 51.0) {
		float speed = 0.05;
...
#ifdef WAVING_BEETROOT_SEEDS
	if (mc_Entity.x == 244.0) {
		float speed = 0.05;
...
//*imagine irish accent* potatoes are my life, potatoes are what gave my family it's money, and now it's all gone, CURSE YOU POTATO FAMINE
...
#ifdef WAVING_MELON
	if (mc_Entity.x == 103.0) {
		float speed = 0.1;


			  //lightWeight = max(0.0f, 1.0f - (lightWeight * 5.0f));
		
		float magnitude = (sin((position.y + position.x + tick * pi / ((8.0) * speed))) * 0.15 + 0.15) * 0.30 * lightWeight;
			  // magnitude *= grassWeight;
			  magnitude *= lightWeight;
		float d0 = sin(tick * pi / (112.0 * speed)) * 3.0 - 1.5;
		float d1 = sin(tick * pi / (142.0 * speed)) * 3.0 - 1.5;
